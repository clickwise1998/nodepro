...
function AtDCore() {
	...
	this.i18n = {};
        ...
        /*store current errors in order of occurence in the text*/
        this.currentErrorList=new Array();
...
AtDCore.prototype.makeError = function(error_s, tokens, type, seps, pre) {
	...
	struct.tokens = tokens;
        struct.pre=pre;
        var dngram=pre+" "+error_s;
        struct.regexpTest=new RegExp(dngram);
	if (new RegExp("\\b" + error_s + "\\b").test(error_s)) {
		struct.regexp = new RegExp("(?!"+error_s+"<\\/span)\\b"+pre+" (" + error_s.replace(/\s+/g, seps) + ")\\b");
                struct.regexpCurr = new RegExp("(?!"+error_s+"<\\/span)\\b"+ error_s.replace(/\s+/g, seps) + "\\b");
	}
	else if (new RegExp(error_s + "\\b").test(error_s)) {
		struct.regexp = new RegExp("(?!"+error_s+"<\\/span)" +pre+" (" + error_s.replace(/\s+/g, seps) + ")\\b");
                struct.regexpCurr = new RegExp("(?!"+error_s+"<\\/span)" + error_s.replace(/\s+/g, seps) + "\\b");
	}
	else if (new RegExp("\\b" + error_s).test(error_s)) {
		struct.regexp = new RegExp("(?!"+error_s+"<\\/span)\\b" +pre+" (" + error_s.replace(/\s+/g, seps)+")");
                struct.regexpCurr = new RegExp("(?!"+error_s+"<\\/span)\\b" + + error_s.replace(/\s+/g, seps)); 
	}
	else {
		struct.regexp = new RegExp("(?!"+error_s+"<\\/span)" + pre+" (" +error_s.replace(/\s+/g, seps)+")");
                struct.regexpCurr = new RegExp("(?!"+error_s+"<\\/span)" +error_s.replace(/\s+/g, seps));
	}
...
AtDCore.prototype.addToCurrErrorList=function(suggestion,currentText){
        ...
        if(exist==false){
           this.currentErrorList.push(suggestion);
        }

        this.currentErrorList.sort(function(s,t){
            var a=s["orderIndex"];
            var b=t["orderIndex"];
            if(a<b) return -1;
            if(a>b) return 1;
        ...
        var currIndex=1000000;
        
        for(var i=0;i<this.currentErrorList.length;i++){
            currIndex=this.currentErrorList[i]["orderIndex"];
            if(currIndex==-1){
               this.currentErrorList.shift();
            }
        }
        ...
        var itemDescp="";
        for(var i=0;i<this.currentErrorList.length;i++){
          itemDescp=this.currentErrorList[i]["orderIndex"]+" "+this.currentErrorList[i]["context"]+" "+this.currentErrorList[i]["string"];
          errorsDesp=errorsDesp+" ["+itemDescp+"] ";
        }
         
        /*alert("errorContKey:"+errorContKey+" orderIndex:"+suggestion["orderIndex"]+" exist:"+exist+" errorsDesp:"+errorsDesp+" currentText:"+currentText);*/
};

AtDCore.prototype.removeRedundantBlank = function(str){
        return str.replace(/^[\s]+/,'').replace(/[\s]+$/,'').replace(/[\s]{2,}/,' ');
};

AtDCore.prototype.clearCurrentErrors = function(){
       this.currentErrorList.splice(0,this.currentErrorList.length);
       $('#error-list').empty();
};

AtDCore.prototype.processXML = function(responseXML,currentText) {
       
        currentText=this.removeRedundantBlank(currentText);
				...
				if (errors[i].getElementsByTagName('suggestions').item(0) != undefined) {
					...
					for (var j = 0; j < suggestions.length; j++)
						suggestion["suggestions"].push(suggestions[j].firstChild.data);
                                        ...
                                        {
                                          /*$('#error-list').append(errorString+"->"+suggestion["suggestions"][0]+"<br/>");*/
                                          /* $('#error-list').append("<font color=\"red\">"+errorString+"</font>-><font color=\"#00CC33\">"+suggestion["suggestions"][0]+"</font><br/>");*/
                                        }
				}
...
AtDCore.prototype.findSuggestion = function(element) {
        /*alert("in atd core find suggestion");*/
...
AtDCore.prototype.markMyWords = function(container_nodes, errors) {
	...
	/* Our goal--ignore nodes that are already wrapped */
       ...
       for(j=0;j<container_nodes.length;j++)
       {
           if(container_nodes[j]==null||container_nodes[j].nodeValue==null)
              continue;
           if(j!=container_nodes.length-1)
	...
	/* walk through the relevant nodes */
        /*why second loop nl.length become 9 from 1*/
	var iterator;

	this.map(nl, function(n) {
		var v;
                /*text nodes has value 3*/
		if (n.nodeType == 3) {
			...
			while (iterator.hasNext()) {
				...
				if (current != undefined && current.pretoks != undefined) {
                                        /*alert("token:"+token+" current:"+current+" previous:"+previous);*/
					...
					prev = v.substr(0, iterator.getCount());
                                        ...
                                        /*alert(" current toks:"+current+" prev:"+prev+" curr:"+curr+" error.regexp:"+error.regexp);*/
					var checkErrors = function(error) {
                                           /*alert(" current toks:"+current+" prev:"+prev+" curr:"+curr+" error.regexp:"+error.regexp);*/
						if (error != undefined && !error.used && foundStrings['__' + error.string] == undefined && error.regexpCurr.test(curr)) {
							var oldlen = curr.length;
                                                        var preCom=error['pre']+ ' ';
                                                        
                                                        /*alert("test push error.string:"+error.string+" previous:["+previous+"] pre:["+error['pre']+"] preCom:"+preCom);*/
                                                        if(preCom==previous)
                                                        {
                                                          /*alert("push error.string:"+error.string+" previous:["+previous+"] pre:["+error['pre']+"] preCom:"+preCom);*/
							  foundStrings['__' + error.string] = 1;
							  doReplaces.push([error.regexp,error['pre']+ ' <span class="'+error.type+'" pre="'+previous+'">$1</span>',error.regexpTest]);
							  error.used = true;
							  done = true;
                                                        }
						}
			...
			if (doReplaces.length > 0) {
				...
				for (var x = 0; x < doReplaces.length; x++) {
					var regexp = doReplaces[x][0], result = doReplaces[x][1],regexpTest=doReplaces[x][2];
                                        /*alert("x="+x+" regexp:"+regexp+" result:"+result+" regexpTest:"+regexpTest);*/
					...
					var bringTheHurt = function(node) {
                                                /*alert("parent:"+parent.nodeValue+" node:"+node.nodeValue+" node.nodeType:"+node.nodeType);*/
						if (node.nodeType == 3) {//initial replace?
							ecount++;
							...
							var contents = parent.contents(node);
                                                        /*alert("contents:"+contents+" contents.length:"+contents.length);*/
							for (var y = 0; y < contents.length; y++) {
                                                                /*alert("x="+x+" y="+y+" type:"+contents[y].nodeType+" value:"+contents[y].nodeValue);*/
								/*if (contents[y].nodeType == 3 && regexp.test(contents[y].nodeValue)) {*/
                                                                if (contents[y].nodeType == 3 && regexpTest.test(contents[y].nodeValue)) {
									var nnode;
							...
							return node;
                                        ...
                                        /*alert("x="+x+" before nodeType:"+newNode.nodeType+" newNode:"+newNode.nodeValue+" nh:"+newNode.innerText);*/
					newNode = bringTheHurt(newNode);
                                        /*alert("x="+x+" after nodeType:"+newNode.nodeType+" newNode:"+newNode.nodeValue+" nh:"+newNode.innerText);*/
				}
                                /*alert("ulimate replace: nodeType:"+newNode.nodeType+" nodeValue:"+newNode.nodeValue);*/
...
AtDCore.prototype._findTextNodes=function(e) {
  alert("e:"+e);
...
AtDCore.prototype._walk = function(elements,allText, f) {
	var i;

        /*here we only check the first textnode ,why?*/
	/*for (i = 0; i < elements.length; i++) {*/
       /**        
        for (i = 0; i < 1&&i<elements.length; i++) {
                if(i==0)
                {
                  elements[0].nodeValue=allText;
                }
		f.call(f, elements[i]);
		this._walk(this.contents(elements[i]),allText, f);
	}**/
	for (i = 0; i < elements.length; i++) {
		f.call(f, elements[i]);
		this._walk(this.contents(elements[i]),allText, f);
...
AtdCore.prototype.show=function(cont){
        alert("cont:"+cont);
};
