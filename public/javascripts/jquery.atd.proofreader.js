...
function AtD_Proofreader(container,socket) {
	...
	AtD.sync = function() {
                /*alert("in jquery.atd.proofreader sync");*/
		if (AtD.isProofreading) {
			...
			window.setTimeout(function() {
				...
				if (current >= 1750 && AtD.isProofreading && !AtD.transition) {
					...
					var content = clone.html();
                                        /*alert("original:"+original+" content:"+content);*/
					if (content != null)
	...
	AtD.checkComponent = function() {
                AtD.socket=socket;
                /*alert("try to checkComponent in jquery.atd.proofreader");*/
		/* If the text of the link says edit comment, then restore the textarea so the user can edit the text */
        	if (isProofreading) {
                          alert("not begin to checkComponent in jquery.atd.proofreader");
			AtD.restore();
                        $('#error-list').empty();
		}
		else {
			AtD.position = AtD.getScrollPosition();
                        /*alert("begin to checkComponent in jquery.atd.proofreader");*/
			/* we're now proofreading */
			isProofreading = true;

			widget.mode("proofread");
                        /*alert("begin to check");*/
			...
			widget.adjustWidget();
                        ...
                        /*alert("the check content:"+div.parent().find('#AtD_Content'));*/
			/* check the writing in the textarea */
			AtD.check(div.parent().find('#AtD_Content'),div, AtD.getCheckValue(container), {
				ready: function(errorCount) {
