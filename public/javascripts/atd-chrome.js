...
jQuery.fn.addProofreader = function(socket,options) {
	this.id = 0;
	...
	return this.each(function() {
		...
		if ($this.context.tagName == 'DIV' && AtD_DIV_isExempt($this) == false) {
			...
			proofreader = new AtD_Proofreader($this,socket);
		}
                /*alert("begin to attach");*/
                /*alert(this);*/
                this.addEventListener("oninput",function(){alert("test show");},true);
                this.addEventListener("onpropertychange",function(){alert("test show");},true);
                $this.bind('propertychange input',function(){alert("test show bind");});
